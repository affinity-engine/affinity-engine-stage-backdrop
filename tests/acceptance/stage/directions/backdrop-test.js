...
test('Affinity Engine | stage | Directions | Backdrop', function(assert) {
  ...
  visit('/affinity-engine/test-scenarios/stage/directions/backdrop').then(() => {
    ...
    assert.ok(Ember.$(`${hook('backdrop_direction')} img`).attr('src').match('engine/backdrops/classroom.png'), 'it sets the `src` based on the associated fixture');
    ...
    assert.ok(Ember.$(`${hook('backdrop_direction')}:nth(3) img`).attr('src').match('engine/backdrops/beach-night.jpg'), 'the manually defined backdrop src is set properly');
  });
});
