...
test('Affinity Engine | stage | Directions | Backdrop', function(assert) {
  ...
  visit('/').then(() => {
    ...
    assert.equal(parseFloat($hook('affinity_engine_stage_direction_backdrop').css('opacity')).toFixed(1), 0.2, '`transition` sets backdrop css');

    return step(100);
  }).then(() => {
    assert.equal(parseFloat($hook('affinity_engine_stage_direction_backdrop').css('opacity')).toFixed(1), 0.5, '`transition`s can be chained');
    ...
    assert.equal(parseFloat(Ember.$(`${hook('affinity_engine_stage_direction_backdrop')}:first`).css('opacity')).toFixed(1), 0.5, 'instances respond independently to `transition`s: 1');
    assert.equal(parseFloat(Ember.$(`${hook('affinity_engine_stage_direction_backdrop')}:nth(1)`).css('opacity')).toFixed(1), 0.6, 'instances respond independently to `transition`s: 2');
